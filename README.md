...
| 🤖 **模型灵活配置** | 支持多模型并行，用户可实时检测可用模型 |
| 📝 **LaTeX输出** | 直接生成学术出版级别的LaTeX格式 |
| 📊 **智能PDF解析** | 使用Docling AI解析学术论文，精确提取章节 |
...
- **API Key**: 从模型中转站获取的API密钥
- **用途**: 用于论文写作的主要AI模型调用
- **自动检测**: 填写API信息后，系统会自动检测可用的模型列表
...
在侧边栏配置API后，系统会自动检测可用的模型并显示在下拉列表中。
...
### Tab 1: 智能风格分析
上传目标期刊范文PDF，系统进行深度分析：
- **智能PDF解析**: 使用Docling AI（IBM）精确提取PDF章节内容
...
#### 🎯 工作流程：
1. **文档解析**: 智能提取研究内容，**GPT-4o Vision**分析图表数据
2. **数据确认**: 验证提取的文本、表格、图片分析结果
3. **文献选择**: 多选文献库，系统自动整合
4. **模型配置**: 为各章节选择最适合的AI模型

#### 🔒 隐私与数据安全
- **Word文档处理**: 使用GPT-4o Vision进行图片和图表分析，确保数据隐私
- **结果数据建议**: 为保护原始数据隐私，建议只上传结果描述而非原始数据集
- **本地处理**: 所有分析在本地完成，不上传用户数据到第三方服务器
5. **AI协作写作**:
...
1. **研究内容准备**
   ...
   - **结果部分**: 为保护数据隐私，建议只上传结果描述而非原始数据集
   - 图表可以直接粘贴到Word文档中，系统会自动分析
...
#### 一级AI：智能规划师
- **角色**: SkillGeneratorAgent (基于Claude-3.5 Sonnet)
...
### AI模型支持
- **openai >= 1.0.0** - OpenAI GPT模型 (文档分析用GPT-4o)
- **anthropic-sdk >= 0.2.0** - Claude模型 (一级AI用Claude-3.5 Sonnet)
